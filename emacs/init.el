...
;; Display CPU load, date and time in mode bar
...
(require 'dired+) ; Make dired+ re-use buffer for visited directories
...
(setq initial-scratch-message "") ;; No initial *scratch* buffer
(set-face-attribute 'default nil :height 105) ;; Set custom font
...
(custom-set-faces
 ...
 '(helm-candidate-number ((t (:background "sienna" :foreground "black"))))
 '(helm-selection ((t (:background "gray5" :distant-foreground "black"))))
 '(helm-source-header ((t (:background "dim gray" :foreground "white" :weight bold :family "Sans Serif"))))
 '(helm-visible-mark ((t (:background "olive drab" :foreground "black"))))
...
(global-set-key (kbd "C-c C-c y") 'popup-kill-ring)
...
(global-set-key (kbd "C-x a")  'mark-whole-buffer) ;; Rebind the rebound C-x h to C-x a to mark-whole-buffer    
...
;(global-set-key (kbd "C-x C-b") 'buffer-menu) ;; Open buffer list in active frame
(global-set-key (kbd "C-c m")  'menu-bar-mode) ;; Set keys to toggle menu
(global-set-key (kbd "C-x C-b") 'ibuffer) ;; Set C-x C-b to Ibuffer and set a few other things
;;(global-set-key (kbd "C-x C-B") 'ibuffer-other-window)
(setq ibuffer-default-sorting-mode 'major-mode)
(global-set-key (kbd "C-c y")  'popup-kill-ring) ;; Bind C-c y
...
(setq evil-move-cursor-back nil) ;; Don't move cursor back on when exiting insert mode

(setq gdb-many-windows t) ;; Enable multiple GDB windows

(desktop-save-mode 1) ;; Save the and restore session'

(add-hook 'python-mode-hook 'electric-indent-mode) ;; Enable electric-indent-mode for python-mode

(setq Buffer-menu-sort-column 4) ;; Sort buffer-menu cols sort on filename

(setq highlight-indentation-mode nil) ;; Set indent highlights off
...
(setq x-select-enable-clipboard t) ;; Enable clipboard copypasta
